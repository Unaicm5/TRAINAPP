...
export default function ClientDetail({ clientId, navTo, showToast, session }) {
  ...
  const registerSession = async () => {
    const { error } = await supabase.from('sessions').update({ done: true, ...regForm }).eq('id', registerModal)
    if (!error) { showToast('Sesión registrada ✓'); setRegisterModal(null); loadData() }
  }
  ...
  return (
    <div>
          ...
          {sessions.length === 0
            ...
            : sessions.map(s => (
              <div key={s.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 16, marginBottom: 8 }}>
                ...
                {s.client_notes && <div style={{ background: 'rgba(245,158,11,0.06)', borderLeft: '2px solid var(--amber)', borderRadius: '0 var(--r-xs) var(--r-xs) 0', padding: '8px 12px', marginTop: 8, fontSize: 12, color: '#fcd34d' }}>"{s.client_notes}"</div>}
              </div>
            ))}
        </div>
      )}
      ...
      <Modal title="Nueva sesión" open={sessionModal} onClose={() => setSessionModal(false)}>
        ...
        <FormGroup label="Links YouTube (uno por línea)">
          <textarea value={nsForm.videos} onChange={e => setNsForm({ ...nsForm, videos: e.target.value })} style={{ height: 60 }} placeholder="https://youtube.com/shorts/..." />
        </FormGroup>
        <FormGroup label="Notas"><textarea value={nsForm.notes} onChange={e => setNsForm({ ...nsForm, notes: e.target.value })} placeholder="Indicaciones para el cliente..." /></FormGroup>
