...
export default function ClientDetail({ clientId, navTo, showToast, session }) {
  ...
  const loadData = async () => {
    ...
    setClient(c); setSessions(s || []); setVals(v || []); setObjs(o || []); setLoading(false)
  }

  const registerSession = async () => {
    await supabase.from('sessions').update({ done: true, ...regForm }).eq('id', registerModal)
    showToast('Sesión registrada ✓'); setRegisterModal(null); setView('detail'); setOpenSession(null); loadData()
  }
  ...
  const toggleValVis = async (id, visible) => { await supabase.from('valoraciones').update({ visible: !visible }).eq('id', id); loadData() }
  const toggleObjVis = async (id, visible) => { await supabase.from('objetivos').update({ visible: !visible }).eq('id', id); loadData() }

  const calcPct = (o) => {
    const start = parseFloat(o.start_val)||0, target = parseFloat(o.target_val)||1, current = parseFloat(o.current_val)||0
  ...
  if (view==='importar') return <Importar session={session} showToast={showToast} navTo={navTo} clienteId={clientId} clienteNombre={client.name} clienteColor={client.color} onBack={()=>{setView('detail');loadData()}} />
  if (view==='session'&&openSession) return <SessionView session={openSession} showToast={showToast} onBack={()=>{setView('detail');setOpenSession(null)}} onRegister={()=>setRegisterModal(openSession.id)} />
  ...
  const currentMicro=(lastSession?.micro||'—').replace(/MICROCICLO /i,'MC').replace(/Microciclo /i,'MC')
  const currentMeso=(lastSession?.mesociclo||'—').replace(/MESOCICLO /i,'MS').replace(/Mesociclo /i,'MS')
    ...
    <div>
      ...
      {tab==='sesiones'&&(
        <div>
          {sessions.length===0
            ...
            :sessions.map(s=>(
              <div key={s.id} onClick={()=>{setOpenSession(s);setView('session')}} style={{background:'var(--bg2)',border:`1px solid ${s.done?'var(--gborder)':'var(--border)'}`,borderRadius:'var(--r)',padding:'14px 16px',marginBottom:8,cursor:'pointer',display:'flex',alignItems:'center',gap:12}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color:'var(--accent)',fontFamily:'DM Mono,monospace',marginBottom:2}}>{s.micro}</div>
                ...
                <div style={{display:'flex',gap:6,alignItems:'center'}}>
                  ...
                  <i className="ti ti-chevron-right" style={{fontSize:16,color:'var(--text3)'}}></i>
                </div>
              </div>
            ))}
        </div>
      ...
      {tab==='valoraciones'&&(
        <div>
          ...
          {cats.length===0?<EmptyState icon="ti-chart-bar" text="Sin valoraciones"/>:cats.map(cat=>(
            <div key={cat}>
              ...
              <Card style={{marginBottom:8,padding:'0 20px'}}>
                {vals.filter(v=>v.category===cat).map((v,i,arr)=>(
                  <div key={v.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 0',borderBottom:i<arr.length-1?'1px solid var(--border)':'none'}}>
                    <div style={{flex:1}}><div style={{fontSize:13,color:'var(--text2)'}}>{v.name}</div><div style={{fontSize:11,color:'var(--text3)'}}>{formatDate(v.date)}</div></div>
                    <div style={{fontSize:15,fontWeight:600,fontFamily:'Syne,sans-serif',minWidth:70,textAlign:'right'}}>{v.value}</div>
                    <button onClick={e=>{e.stopPropagation();toggleValVis(v.id,v.visible)}} style={{fontSize:10,padding:'2px 8px',borderRadius:99,cursor:'pointer',border:'0.5px solid',background:v.visible?'var(--gdim)':'var(--bg4)',color:v.visible?'var(--green)':'var(--text3)',borderColor:v.visible?'var(--gborder)':'var(--border)',fontFamily:'DM Mono,monospace'}}>{v.visible?'Visible':'Oculto'}</button>
                  </div>
                ))}
              </Card>
            </div>
      ...
      {tab==='objetivos'&&(
        <div>
          ...
          {objs.length===0?<EmptyState icon="ti-target" text="Sin objetivos"/>:objs.map(o=>{
            const pct=calcPct(o),st=objStatus(pct)
            return(
              <div key={o.id} style={{borderRadius:'var(--r)',padding:16,marginBottom:8,border:`1px solid ${st.border}`,background:st.bg}}>
                ...
                <p style={{fontSize:12,color:'var(--text3)',marginBottom:10}}>{o.start_val?`Inicio: ${o.start_val} · `:''}Actual: {o.current_val} · Meta: {o.target_val}{o.deadline?` · Límite: ${formatDate(o.deadline)}`:' · Continuo'}</p>
      ...
      <Modal title="Nuevo objetivo" open={objModal} onClose={()=>setObjModal(false)}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
          <FormGroup label="Tipo"><select value={objForm.type} onChange={e=>setObjForm({...objForm,type:e.target.value})}><option value="deadline">Con fecha límite</option><option value="continuous">Continuo</option></select></FormGroup>
